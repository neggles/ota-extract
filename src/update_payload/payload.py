...
def _ReadInt(file_obj, size, is_unsigned, hasher=None):
    ...
    return struct.unpack(
        common.IntPackingFmtStr(size, is_unsigned), common.Read(file_obj, size, hasher=hasher)
    )[0]
...
class Payload(object):
    ...
    class _PayloadHeader(object):
        ...
        def ReadFromPayload(self, payload_file, hasher=None):
            ...
            self.version = _ReadInt(payload_file, self._VERSION_SIZE, True, hasher=hasher)
            self.manifest_len = _ReadInt(payload_file, self._MANIFEST_LEN_SIZE, True, hasher=hasher)
    ...
    def __init__(self, payload_file, payload_file_offset=0):
        ...
        elif isinstance(payload_file, (str, Path)):
    ...
    def _ReadManifest(self):
        ...
        return common.Read(self.payload_file, self.header.manifest_len, hasher=self.manifest_hasher)
    ...
    def _ReadMetadataSignature(self):
        ...
        return common.Read(
            ...
            offset=self.payload_file_offset + self.header.size + self.header.manifest_len,
        )
    ...
    def ReadDataBlob(self, offset, length):
        ...
        """
        return common.Read(
            self.payload_file, length, offset=self.payload_file_offset + self.data_offset + offset
        )
    ...
    def IsDelta(self):
        ...
        self._AssertInit()
        return any(partition.HasField("old_partition_info") for partition in self.manifest.partitions)
