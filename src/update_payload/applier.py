...
def _VerifySha256(file_obj, expected_hash, name, length=-1):
    ...
    if actual_hash != expected_hash:
        raise PayloadError(
            ...
            % (name, common.FormatSha256(actual_hash), common.FormatSha256(expected_hash))
        )
...
def _WriteExtents(file_obj, data, extents, block_size, base_name):
    ...
    for ex, ex_name in common.ExtentIter(extents, base_name):
        ...
        if ex.start_block != common.PSEUDO_EXTENT_MARKER:
            file_obj.seek(ex.start_block * block_size)
            data_view = io.BytesIO(data, data_offset, write_length)
...
class PayloadApplier(object):
    ...
    def _ApplyReplaceOperation(self, op, op_name, out_data, part_file, part_size):
        ...
        data_start = 0
        for ex, ex_name in common.ExtentIter(op.dst_extents, "%s.dst_extents" % op_name):
            ...
            if start_block != common.PSEUDO_EXTENT_MARKER:
                ...
                if data_end >= data_length + block_size:
                    raise PayloadError("%s: more dst blocks than data (even with padding)")
        ...
        if data_start < data_length:
            raise PayloadError(
                "%s: wrote fewer bytes (%d) than expected (%d)" % (op_name, data_start, data_length)
            )
    ...
    def _ApplyMoveOperation(self, op, op_name, part_file):
        ...
        _WriteExtents(part_file, in_data, op.dst_extents, block_size, "%s.dst_extents" % op_name)
    ...
    def _ApplySourceCopyOperation(self, op, op_name, old_part_file, new_part_file):
        ...
        if not old_part_file:
            raise PayloadError(
                "%s: no source partition file provided for operation type (%d)" % (op_name, op.type)
            )
        ...
        # Dump extracted data to dst extents.
        _WriteExtents(
            new_part_file, in_data, op.dst_extents, block_size, "%s.dst_extents" % op_name
        )
    ...
    def _ApplyDiffOperation(self, op, op_name, patch_data, old_part_file, new_part_file):
        ...
        if hasattr(new_part_file, "fileno") and (
            ...
            if op.type in (
                ...
                # Invoke bspatch.
                bspatch_cmd = [self.bspatch_path, in_file_name, out_file_name, patch_file_name]
                subprocess.check_call(bspatch_cmd)
            elif op.type == common.OpType.PUFFDIFF:
                # Invoke puffpatch.
                puffpatch_cmd = [
            ...
            _WriteExtents(
                new_part_file, out_data, op.dst_extents, block_size, "%s.dst_extents" % op_name
            )
    ...
    def _ApplyOperations(self, operations, base_name, old_part_file, new_part_file, part_size):
        ...
        for op, op_name in common.OperationIter(operations, base_name):
            ...
            if op.type in (
                ...
                self._ApplyDiffOperation(op, op_name, data, new_part_file, new_part_file)
            elif op.type == common.OpType.SOURCE_COPY:
                self._ApplySourceCopyOperation(op, op_name, old_part_file, new_part_file)
            elif op.type in (
                common.OpType.SOURCE_BSDIFF,
                common.OpType.PUFFDIFF,
                common.OpType.BROTLI_BSDIFF,
            ):
                self._ApplyDiffOperation(op, op_name, data, old_part_file, new_part_file)
            else:
                raise PayloadError("%s: unknown operation type (%d)" % (op_name, op.type))
    ...
    def _ApplyToPartition(
        ...
        if old_part_file_name:
            ...
            with open(old_part_file_name, "rb") as old_part_file:
                _VerifySha256(
                    old_part_file, old_part_info.hash, "old " + part_name, length=old_part_info.size
                )
        ...
        with open(new_part_file_name, new_part_file_mode) as new_part_file:
            ...
            try:
                self._ApplyOperations(
                    operations, base_name, old_part_file, new_part_file, new_part_info.size
                )
            finally:
                if old_part_file:
                    old_part_file.close()
        ...
        with open(new_part_file_name, "rb") as new_part_file:
            _VerifySha256(
                new_part_file, new_part_info.hash, "new " + part_name, length=new_part_info.size
            )
    ...
    def Run(self, new_parts, old_parts=None):
        ...
        if self.payload.header.version == 1:
            ...
            install_operations.append((common.KERNEL, manifest.kernel_install_operations))
        else:
        ...
        if new_part_names != part_names:
            raise PayloadError(
                "missing dst partition(s) %s" % ", ".join(part_names - new_part_names)
            )
