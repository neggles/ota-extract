...
from update_metadata import update_metadata_pb2
...
_DEFAULT_PUBKEY_FILE_NAME = os.path.join(os.path.dirname(__file__), _DEFAULT_PUBKEY_BASE_NAME)
...
class PayloadChecker(object):
    ...
    def __init__(self, payload, assert_type=None, block_size=0, allow_unhashed=False, disabled_tests=()):
        ...
        if not _IsPowerOfTwo(self.block_size):
            raise error.PayloadError("Expected block (%d) size is not a power of two." % self.block_size)
    ...
    @staticmethod
    def _CheckElem(
        msg, name, report, is_mandatory, is_submsg, convert=str, msg_name=None, linebreak=False, indent=0
    ):
        ...
        if not msg.HasField(name):
            if is_mandatory:
                raise error.PayloadError(
                    ...
                    % (msg_name + " " if msg_name else "", "sub-message" if is_submsg else "field", name)
                )
    ...
    def _CheckRepeatedElemNotPresent(msg, field_name, msg_name):
        ...
        if getattr(msg, field_name, None):
            raise error.PayloadError(
                "%sfield %r not empty." % (msg_name + " " if msg_name else "", field_name)
            )
    ...
    def _CheckElemNotPresent(msg, field_name, msg_name):
        ...
        if msg.HasField(field_name):
            raise error.PayloadError("%sfield %r exists." % (msg_name + " " if msg_name else "", field_name))

    @staticmethod
    def _CheckMandatoryField(msg, field_name, report, msg_name, convert=str, linebreak=False, indent=0):
    ...
    @staticmethod
    def _CheckOptionalField(msg, field_name, report, convert=str, linebreak=False, indent=0):
        """Adds an optional field; returning first component from _CheckElem."""
        return PayloadChecker._CheckElem(
            msg, field_name, report, False, False, convert=convert, linebreak=linebreak, indent=indent
        )[0]
    ...
    def _CheckPresentIff(val1, val2, name1, name2, obj_name):
        ...
        if None in (val1, val2) and val1 is not val2:
            ...
            raise error.PayloadError(
                "%r present without %r%s." % (present, missing, " in " + obj_name if obj_name else "")
            )
    ...
    def _CheckPresentIffMany(vals, name, obj_name):
        ...
        if any(vals) and not all(vals):
            raise error.PayloadError(
                "%r is not present in all values%s." % (name, " in " + obj_name if obj_name else "")
            )
    ...
    def _Run(cmd, send_data=None):
        ...
        """
        run_process = subprocess.Popen(cmd, stdin=subprocess.PIPE, stdout=subprocess.PIPE)
        try:
    ...
    def _CheckSha256Signature(sig_data, pubkey_file_name, actual_hash, sig_name):
        ...
        if len(sig_data) != 256:
            raise error.PayloadError(
                "%s: signature size (%d) not as expected (256)." % (sig_name, len(sig_data))
            )
        signed_data, _ = PayloadChecker._Run(
            ["openssl", "rsautl", "-verify", "-pubin", "-inkey", pubkey_file_name], send_data=sig_data
        )
        if len(signed_data) != len(common.SIG_ASN1_HEADER) + 32:
            raise error.PayloadError("%s: unexpected signed data length (%d)." % (sig_name, len(signed_data)))
        if not signed_data.startswith(common.SIG_ASN1_HEADER):
            raise error.PayloadError("%s: not containing standard ASN.1 prefix." % sig_name)
        signed_hash = signed_data[len(common.SIG_ASN1_HEADER) :]
        if signed_hash != actual_hash:
            raise error.PayloadError(
                "%s: signed hash (%s) different from actual (%s)."
                % (sig_name, common.FormatSha256(signed_hash), common.FormatSha256(actual_hash))
            )

    @staticmethod
    def _CheckBlocksFitLength(length, num_blocks, block_size, length_name, block_name=None):
    ...
    def _CheckManifestMinorVersion(self, report):
        ...
        if self.minor_version in _SUPPORTED_MINOR_VERSIONS:
            ...
            raise error.PayloadError("Unsupported minor version: %d" % self.minor_version)
    ...
    def _CheckManifest(self, report, part_sizes=None):
        ...
        if actual_block_size != self.block_size:
            raise error.PayloadError(
                "Block_size (%d) not as expected (%d)." % (actual_block_size, self.block_size)
            )
        ...
        self._CheckPresentIff(
            self.sigs_offset, self.sigs_size, "signatures_offset", "signatures_size", "manifest"
        )
        ...
        if is_delta:
            ...
            if self.payload_type == _TYPE_FULL:
                raise error.PayloadError("Apparent full payload contains old_{kernel,rootfs}_info.")
            ...
            for part, (msg, part_report) in self.old_part_info.items():
                ...
                self.old_fs_sizes[part] = self._CheckMandatoryField(msg, "size", part_report, field)
                self._CheckMandatoryField(msg, "hash", part_report, field, convert=common.FormatSha256)
            ...
            if self.payload_type == _TYPE_DELTA:
                raise error.PayloadError("Apparent delta payload missing old_{kernel,rootfs}_info.")
        ...
        for part, (msg, part_report) in self.new_part_info.items():
            ...
            self.new_fs_sizes[part] = self._CheckMandatoryField(msg, "size", part_report, field)
            self._CheckMandatoryField(msg, "hash", part_report, field, convert=common.FormatSha256)
    ...
    def _CheckLength(self, length, total_blocks, op_name, length_name):
        ...
        self._CheckBlocksFitLength(length, total_blocks, self.block_size, "%s: %s" % (op_name, length_name))
    ...
    def _CheckExtents(self, extents, usable_size, block_counters, name):
        ...
        for ex, ex_name in common.ExtentIter(extents, name):
            ...
            start_block = PayloadChecker._CheckMandatoryField(ex, "start_block", None, ex_name)
            num_blocks = PayloadChecker._CheckMandatoryField(ex, "num_blocks", None, ex_name)
    ...
    def _CheckReplaceOperation(self, op, data_length, total_dst_blocks, op_name):
        ...
        if isinstance(total_dst_blocks, float):
            raise error.PayloadError("%s: contains invalid data type of " "total_dst_blocks." % op_name)
        ...
        if op.type == common.OpType.REPLACE:
            PayloadChecker._CheckBlocksFitLength(
                data_length, total_dst_blocks, self.block_size, op_name + ".data_length", "dst"
            )
        else:
            ...
            if data_length >= total_dst_blocks * self.block_size:
                raise error.PayloadError(
                    ...
                    "space (%d * %d)." % (op_name, data_length, total_dst_blocks, self.block_size)
                )
    ...
    def _CheckSourceCopyOperation(self, data_offset, total_src_blocks, total_dst_blocks, op_name):
    ...
    def _CheckOperation(
        ...
        self._CheckPresentIff(data_offset, data_length, "data_offset", "data_length", op_name)
        ...
        if op.HasField("data_sha256_hash"):
            ...
            actual_hash = hashlib.sha256(self.payload.ReadDataBlob(data_offset, data_length))
        ...
        if op.type in (common.OpType.REPLACE, common.OpType.REPLACE_BZ, common.OpType.REPLACE_XZ):
            ...
            self._CheckSourceCopyOperation(data_offset, total_src_blocks, total_dst_blocks, op_name)
    ...
    def _CheckOperations(
        ...
        old_block_counters = self._AllocBlockCounters(old_usable_size) if old_fs_size else None
        ...
        report.AddField(
            None, histogram.Histogram.FromCountDict(op_counts, key_names=common.OpType.NAMES), indent=1
        )
        report.AddField("total blobs", sum(blob_hash_counts.values()))
        report.AddField(None, histogram.Histogram.FromCountDict(blob_hash_counts), indent=1)
        report.AddField("total blob size", _AddHumanReadableSize(total_data_used))
        report.AddField(
            None,
            histogram.Histogram.FromCountDict(
                op_blob_totals, formatter=_AddHumanReadableSize, key_names=common.OpType.NAMES
            ),
            indent=1,
        )
    ...
    def _CheckSignatures(self, report, pubkey_file_name):
        ...
        if last_partition.operations:
            ...
            if (
                ...
                raise error.PayloadError(
                    "It seems like the last operation is the " "signature blob. This is an invalid payload."
                )
        ...
        for sig, sig_name in common.SignatureIter(sigs.signatures, "signatures"):
            ...
            if sig.data:
                self._CheckSha256Signature(sig.data, pubkey_file_name, payload_hasher.digest(), sig_name)
    ...
    def Run(
        ...
        try:
            ...
            if self.payload.header.version not in (1, 2):
                raise error.PayloadError("Unknown payload version (%d)." % self.payload.header.version)
            ...
            for field in ("old_kernel_info", "old_rootfs_info", "new_kernel_info", "new_rootfs_info"):
                self._CheckElemNotPresent(self.payload.manifest, field, "manifest")
            total_blob_size = 0
            for part, operations in (
                (p.partition_name, p.operations) for p in self.payload.manifest.partitions
            ):
