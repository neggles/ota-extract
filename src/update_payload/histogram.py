...
class Histogram(object):
    ...
    def FromCountDict(count_dict, scale=20, formatter=None, key_names=None):
        ...
        """
        if key_names:
            hist = [(key_names[key], count) for key, count in count_dict.items()]
        else:
            hist = [(key, count) for key, count in count_dict.items()]
    ...
    def __str__(self):
        ...
        for key, count in self.data:
            ...
            line = "%s %s %s" % (str(key).ljust(self.max_key_len), hist_bar, self.formatter(count))
